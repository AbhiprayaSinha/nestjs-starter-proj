...
import { matchDataType } from 'src/validators/settings.validator';
...
export class SettingService {
    ...
    async create(setting: CreateSettingDto): Promise<Setting> {
        if(!matchDataType(setting.data_type, setting.value)) {
            throw new BadRequestException('The data type of the value does not match the data type of the setting');
        }
